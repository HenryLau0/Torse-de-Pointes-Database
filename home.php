...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (isset($_GET['action']) && $_GET['action'] == "edit") {
        ...
        $query = "UPDATE torse_de_pointes_cases_t SET Source='$Source', PMID='$PMID' WHERE `Case ID`='$id'";

        if (mysqli_query($conn, $query)) {
            header("location: home.php?file=$file");
            exit();
        } else {
            echo "Error: " . mysqli_error($conn);
        }
    } else {
        $query = "INSERT INTO torse_de_pointes_cases_t (Source, PMID, date_posted) 
              VALUES ('$Source', '$PMID', '$date')";
...
<h2 align="center" style="font-weight:bold; font-family: 'Trirong',serif">Torsade de Pointes Cases</h2>
...
if ($_SERVER['REQUEST_METHOD'] == "GET" && isset($_GET['action'])) {
    if ($_GET['action'] == "edit") {
        ...
        $query = "SELECT * FROM torse_de_pointes_cases_t WHERE `Case ID`='$id'";
        $result = mysqli_query($conn, $query);

        if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $editSource = isset($row['Source']) ? $row['Source'] : '';
            ...
            <div class="form-container">
                <form action="home.php?action=edit&id=<?= $id ?>" method="POST">
                    <label for="source">Source:</label>
                    <input type="text" id="source" name="Source" value="<?= $editSource ?>" required><br><br>
                    <label for="pmid">PMID:</label>
                    <input type="text" id="pmid" name="PMID" value="<?= $editPMID ?>" required><br><br>
                    <input type="submit" value="Update"><br><br>
                    <button type="button" onclick="window.location.href='home.php'">Cancel</button><br><br>
                </form>
            </div>
        ...
        <script>
            ...
            if (result) {
                window.location.href = "home.php?action=confirm_delete&id=<?= $id ?>";
            } else {
        ...
        <?php
                ...
                $query = "DELETE FROM torse_de_pointes_cases_t WHERE `Case ID`='$id'";
        ...
        $query = "SELECT * FROM torse_de_pointes_cases_t LIMIT $offset, $recordsPerPage"; // SQL Query
        $result = mysqli_query($conn, $query);
        $resultCheck = mysqli_num_rows($result);
        ?>

<form action="home.php" method="POST" style="max-width:400px; padding:20px; margin: 0 auto" class="w3-container w3-card-4 w3-light-grey w3-text-blue w3-margin">
    <div id="pmidSection" class="w3-row w3-section">
        Add PMID: <input type="text" name="PMID" id="PMID" onchange="showSourceTextbox()" required/><br/>
    </div><br>

    <div id="sourceSection" style="display: none;" class="w3-row w3-section">
        Source: <input type="text" name="Source" id="Source" required/><br><br>
...
<table class="table" border="1px" width="100%">
    ...
    if ($resultCheck > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<td align="center">' . (isset($row['Source']) ? $row['Source'] : '') . "</td>";
            ...
            echo '<td align="center"><a href="home.php?action=edit&id=' . (isset($row['Case ID']) ? $row['Case ID'] : '') . '&page=' . $currentPage . '" class="edit-button">Edit</a></td>';
            echo '<td align="center"><a href="home.php?action=delete&id=' . (isset($row['Case ID']) ? $row['Case ID'] : '') . '&page=' . $currentPage . '" class="delete-button">Delete</a></td>';
            echo "</tr>";
        }
    } else {
...
<div class="d-flex justify-content-center mt-4">
    <?php
    // Pagination
    $query = "SELECT COUNT(*) AS total FROM torse_de_pointes_cases_t";
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_assoc($result);
